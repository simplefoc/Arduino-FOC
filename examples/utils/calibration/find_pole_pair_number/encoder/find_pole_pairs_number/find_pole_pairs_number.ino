 ...
 *
 * To run it just set the correct pin numbers for the BLDC driver and encoder A
 * and B channel as well as the encoder PPR value.
 *
 * The program will rotate your motor a specific amount and check how much it
 * moved, and by doing a simple calculation calculate your pole pair number. The
 * pole pair number will be outputted to the serial terminal.
 *
 * If the pole pair number is well estimated your motor will start to spin in
 * voltage mode with 2V target.
 *
 * If the code calculates negative pole pair number please invert your encoder A
 * and B channel pins or motor connector.
 *
...
void setup() {
  ...
  if (pp <= 0) {
    ...
    Serial.println(F(" - Try changing the search_voltage value or "
    ...
    Serial.println(F(" - You can also try to adjust the target voltage using "
                     "serial terminal!"));
  }
