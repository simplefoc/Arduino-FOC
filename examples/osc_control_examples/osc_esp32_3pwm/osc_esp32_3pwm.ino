 ...
 *
 * It's a nice way to work, easier than changing speeds via Seerial text input.
 * It could also be the basis for a functional UI, for example in a lab,
 * art-gallery or similar situation.
 *
 * For this example we used an ESP32 to run the code, a AS5048B I2C absolute
 * encoder and a generic L298 driver board to drive a Emax 4114 gimbal motor.
 * But there is no reason the OSC part will not work with any other setup.
 *
 * You will need:
 *
 * - a working SimpleFOC setup - motor, driver, encoder
 * - a MCU with WiFi and UDP support, for example an ESP32, or an Arduino with
 * Ethernet Shield
 ...
 * There is an example UI file that works with this sketch, see
 * "layout1.touchosc" You can open the UI file in 'TouchOSC Editor' (free
 * program) and transfer it to the TouchOSC app on your device.
 *
 * Alternatively, there are other OSC UIs which may work, e.g.
 * http://opensoundcontrol.org/implementations
 *
 * Using:
 *
 * Change the values below to match the WiFi ssid/password of your network.
 * Load and run the code on your ESP32. Take a note of the IP address of your
 * ESP32. Load and run the UI in TouchOSC. Configure TouchOSC to connect to your
 * ESP32. The first command you send will cause the ESP32 to start sending
 * responses to your TouchOSC device. After this you will see motor position and
 * speed in the UI. Have fun controlling your SimpleFOC motors from your
 * smartphone!
 *
 */
...
const unsigned int inPort = 8000;  // local port to listen for UDP packets
                                   // (here's where we send the packets)
...
void loop() {
  ...
  if (size > 0) {
    ...
    if (now - lastSend > 100) {
      ...
      if (ang < 0)
        ang = 360 - abs(ang);
      // BOSCBundle's add' returns the OSCMessage so the message's 'add' can be
      // composed together
