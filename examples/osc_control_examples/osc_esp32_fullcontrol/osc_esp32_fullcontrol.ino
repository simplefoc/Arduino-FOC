 ...
 *
 * In this example, all the commands available on the serial command interface
 * are also available via OSC. Also, the example is for 2 motors. If you have
 * only 1 motor, comment out the lines for the second motor.
 *
 *
 *
 *
...
void setup() {
  ...
  // and you don't want current flowing through the other motor while it
  // happens...
  pinMode(0, OUTPUT);
  pinMode(4, OUTPUT);
  pinMode(16, OUTPUT);
...
template <typename T> void sendMessage(String &addr, T datum) {
...
void motorCmd(OSCMessage &msg, int offset, BLDCMotor &motor, float *set_point,
              String &prefix) {
  ...
  if (msg.fullMatch("/P", offset)) {
    ...
    *set_point = getNumber(msg, 0);
    sendMessage(prefix + "/3",
                *set_point); // TODO is the set-point the same as motor.target?
...
void sendMotorMonitoring() {
  ...
  // TODO pack it into one message
  // bundleOUT.add("/M2/i").add(motor2.voltage_q).add(motor2.shaft_velocity).add(motor2.shaft_angle).add(motor2.target);
  Udp.beginPacket(outIp, outPort);
  bundleOUT.send(Udp);
  Udp.endPacket();
  bundleOUT.empty(); // empty the bundle to free room for a new one
...
void loop() {
  ...
  if (size > 0) {
    ...
    if (!bundleIN.hasError()) {
      bundleIN.route(
          "/M1",
          [](OSCMessage &msg, int offset) {
            motorCmd(msg, offset, motor1, &set_point1, m1Prefix);
          },
          0);
      bundleIN.route(
          "/M2",
          [](OSCMessage &msg, int offset) {
            motorCmd(msg, offset, motor2, &set_point2, m2Prefix);
          },
          0);
