...
StepperDriver2PWM::StepperDriver2PWM(int _pwm1, int _in1a, int _in1b, int _pwm2,
                                     int _in2a, int _in2b, int en1, int en2) {
  // Pin initialization
...
StepperDriver2PWM::StepperDriver2PWM(int _pwm1, int _dir1, int _pwm2, int _dir2,
                                     int en1, int en2) {
  // Pin initialization
  pwm1 = _pwm1;  // phase 1 pwm pin number
...
void StepperDriver2PWM::setPwm(float Ua, float Ub) {
  ...
  if (_isset(dir1b))
    digitalWrite(dir1b, Ua <= 0 ? LOW : HIGH);
  // phase 2 direction
  digitalWrite(dir2a, Ub >= 0 ? LOW : HIGH);
  if (_isset(dir2b))
    digitalWrite(dir2b, Ub <= 0 ? LOW : HIGH);
