...
BLDCDriver6PWM::BLDCDriver6PWM(int phA_h, int phA_l, int phB_h, int phB_l,
                               int phC_h, int phC_l, int en) {
  // Pin initialization
  pwmA_h = phA_h;
  pwmB_h = phB_h;
  pwmC_h = phC_h;
...
int BLDCDriver6PWM::init() {
  ...
  return _configure6PWM(pwm_frequency, dead_zone, pwmA_h, pwmA_l, pwmB_h,
                        pwmB_l, pwmC_h, pwmC_l);
}
...
void BLDCDriver6PWM::setPwm(float Ua, float Ub, float Uc) {
  ...
  // hardware specific function - depending on driver and mcu
  _writeDutyCycle6PWM(dc_a, dc_b, dc_c, dead_zone, pwmA_h, pwmA_l, pwmB_h,
                      pwmB_l, pwmC_h, pwmC_l);
}
