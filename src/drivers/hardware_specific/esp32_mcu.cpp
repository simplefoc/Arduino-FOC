...
bldc_3pwm_motor_slots_t esp32_bldc_3pwm_motor_slots[4] = {
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_A, MCPWM0A, MCPWM1A,
     MCPWM2A}, // 1st motor will be MCPWM0 channel A
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_B, MCPWM0B, MCPWM1B,
     MCPWM2B}, // 2nd motor will be MCPWM0 channel B
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_A, MCPWM0A, MCPWM1A,
     MCPWM2A}, // 3rd motor will be MCPWM1 channel A
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_B, MCPWM0B, MCPWM1B,
     MCPWM2B} // 4th motor will be MCPWM1 channel B
};

// define stepper motor slots array
bldc_6pwm_motor_slots_t esp32_bldc_6pwm_motor_slots[2] = {
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_A, MCPWM_OPR_B, MCPWM0A,
     MCPWM1A, MCPWM2A, MCPWM0B, MCPWM1B,
     MCPWM2B}, // 1st motor will be on MCPWM0
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_A, MCPWM_OPR_B, MCPWM0A,
     MCPWM1A, MCPWM2A, MCPWM0B, MCPWM1B,
     MCPWM2B}, // 1st motor will be on MCPWM0
};
...
stepper_4pwm_motor_slots_t esp32_stepper_4pwm_motor_slots[2] = {
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_A, MCPWM_OPR_B, MCPWM0A,
     MCPWM1A, MCPWM0B, MCPWM1B}, // 1st motor will be on MCPWM0
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_A, MCPWM_OPR_B, MCPWM0A,
     MCPWM1A, MCPWM0B, MCPWM1B}, // 1st motor will be on MCPWM1
};

// define 2pwm stepper motor slots array
stepper_2pwm_motor_slots_t esp32_stepper_2pwm_motor_slots[4] = {
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_A, MCPWM0A,
     MCPWM1A}, // 1st motor will be MCPWM0 channel A
    {_EMPTY_SLOT, &MCPWM0, MCPWM_UNIT_0, MCPWM_OPR_B, MCPWM0B,
     MCPWM1B}, // 2nd motor will be MCPWM0 channel B
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_A, MCPWM0A,
     MCPWM1A}, // 3rd motor will be MCPWM1 channel A
    {_EMPTY_SLOT, &MCPWM1, MCPWM_UNIT_1, MCPWM_OPR_B, MCPWM0B,
     MCPWM1B} // 4th motor will be MCPWM1 channel B
};
...
void _configureTimerFrequency(long pwm_frequency, mcpwm_dev_t *mcpwm_num,
                              mcpwm_unit_t mcpwm_unit,
                              float dead_zone = NOT_SET) {
  ...
  pwm_config.frequency =
      2 * pwm_frequency; // set the desired freq - just a placeholder for now
                         // https://github.com/simplefoc/Arduino-FOC/issues/76
  mcpwm_init(mcpwm_unit, MCPWM_TIMER_0,
             &pwm_config); // Configure PWM0A & PWM0B with above settings
  mcpwm_init(mcpwm_unit, MCPWM_TIMER_1,
             &pwm_config); // Configure PWM1A & PWM1B with above settings
  mcpwm_init(mcpwm_unit, MCPWM_TIMER_2,
             &pwm_config); // Configure PWM2A & PWM2B with above settings

  if (_isset(dead_zone)) {
    // dead zone is configured
    float dead_time = (float)(_MCPWM_FREQ / (pwm_frequency)) * dead_zone;
    mcpwm_deadtime_enable(mcpwm_unit, MCPWM_TIMER_0,
                          MCPWM_ACTIVE_HIGH_COMPLIMENT_MODE, dead_time / 2.0,
                          dead_time / 2.0);
    mcpwm_deadtime_enable(mcpwm_unit, MCPWM_TIMER_1,
                          MCPWM_ACTIVE_HIGH_COMPLIMENT_MODE, dead_time / 2.0,
                          dead_time / 2.0);
    mcpwm_deadtime_enable(mcpwm_unit, MCPWM_TIMER_2,
                          MCPWM_ACTIVE_HIGH_COMPLIMENT_MODE, dead_time / 2.0,
                          dead_time / 2.0);
  }
  _delay(100);
  ...
  int prescaler = ceil((double)_MCPWM_FREQ / (double)_PWM_RES_DEF / 2.0 /
  ...
  int resolution_corrected = (double)_MCPWM_FREQ / 2.0 / (double)pwm_frequency /
                             (double)(prescaler + 1);
  // if pwm resolution too low lower the prescaler
  if (resolution_corrected < _PWM_RES_MIN && prescaler > 0)
    resolution_corrected = (double)_MCPWM_FREQ / 2.0 / (double)pwm_frequency /
                           (double)(--prescaler + 1);
...
void _configure2PWM(long pwm_frequency, const int pinA, const int pinB) {
  if (!pwm_frequency || !_isset(pwm_frequency))
    ...
    pwm_frequency = _constrain(pwm_frequency, 0,
                               _PWM_FREQUENCY_MAX); // constrain to 40kHz max
  ...
  for (slot_num = 0; slot_num < 4; slot_num++) {
    if (esp32_stepper_2pwm_motor_slots[slot_num].pin1pwm ==
        _EMPTY_SLOT) { // put the new motor in the first empty slot
...
void _configure3PWM(long pwm_frequency, const int pinA, const int pinB,
  ...
  int slot_num;
  for (slot_num = 0; slot_num < 4; slot_num++) {
    if (esp32_bldc_3pwm_motor_slots[slot_num].pinA ==
        _EMPTY_SLOT) { // put the new motor in the first empty slot
...
// - hardware speciffic
void _configure4PWM(long pwm_frequency, const int pinA, const int pinB,
                    const int pinC, const int pinD) {
  if (!pwm_frequency || !_isset(pwm_frequency))
    pwm_frequency = _PWM_FREQUENCY; // default frequency 25hz
  else
    pwm_frequency = _constrain(pwm_frequency, 0,
  ...
  for (slot_num = 0; slot_num < 2; slot_num++) {
    if (esp32_stepper_4pwm_motor_slots[slot_num].pin1A ==
        _EMPTY_SLOT) { // put the new motor in the first empty slot
...
void _writeDutyCycle2PWM(float dc_a, float dc_b, int pinA, int pinB) {
  ...
  for (int i = 0; i < 4; i++) {
    if (esp32_stepper_2pwm_motor_slots[i].pin1pwm ==
        pinA) { // if motor slot found
      // se the PWM on the slot timers
      // transform duty cycle from [0,1] to [0,100]
      mcpwm_set_duty(
          esp32_stepper_2pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
          esp32_stepper_2pwm_motor_slots[i].mcpwm_operator, dc_a * 100.0);
      mcpwm_set_duty(
          esp32_stepper_2pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
          esp32_stepper_2pwm_motor_slots[i].mcpwm_operator, dc_b * 100.0);
      break;
    }
  }
}
...
void _writeDutyCycle3PWM(float dc_a, float dc_b, float dc_c, int pinA, int pinB,
  ...
  for (int i = 0; i < 4; i++) {
    if (esp32_bldc_3pwm_motor_slots[i].pinA == pinA) { // if motor slot found
      // se the PWM on the slot timers
      // transform duty cycle from [0,1] to [0,100]
      mcpwm_set_duty(esp32_bldc_3pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
                     esp32_bldc_3pwm_motor_slots[i].mcpwm_operator,
                     dc_a * 100.0);
      mcpwm_set_duty(esp32_bldc_3pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
                     esp32_bldc_3pwm_motor_slots[i].mcpwm_operator,
                     dc_b * 100.0);
      mcpwm_set_duty(esp32_bldc_3pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_2,
                     esp32_bldc_3pwm_motor_slots[i].mcpwm_operator,
                     dc_c * 100.0);
      break;
    }
...
void _writeDutyCycle4PWM(float dc_1a, float dc_1b, float dc_2a, float dc_2b,
  ...
  // determine which motor slot is the motor connected to
  for (int i = 0; i < 2; i++) {
    if (esp32_stepper_4pwm_motor_slots[i].pin1A ==
        pin1A) { // if motor slot found
      // se the PWM on the slot timers
      // transform duty cycle from [0,1] to [0,100]
      mcpwm_set_duty(
          esp32_stepper_4pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
          esp32_stepper_4pwm_motor_slots[i].mcpwm_operator1, dc_1a * 100.0);
      mcpwm_set_duty(
          esp32_stepper_4pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
          esp32_stepper_4pwm_motor_slots[i].mcpwm_operator1, dc_1b * 100.0);
      mcpwm_set_duty(
          esp32_stepper_4pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
          esp32_stepper_4pwm_motor_slots[i].mcpwm_operator2, dc_2a * 100.0);
      mcpwm_set_duty(
          esp32_stepper_4pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
          esp32_stepper_4pwm_motor_slots[i].mcpwm_operator2, dc_2b * 100.0);
      break;
    }
  }
}
...
int _configure6PWM(long pwm_frequency, float dead_zone, const int pinA_h,
  ...
  if (!pwm_frequency || !_isset(pwm_frequency))
    pwm_frequency = 20000; // default frequency 20khz - centered pwm has twice
                           // lower frequency
  else
    pwm_frequency =
        _constrain(pwm_frequency, 0,
                   _PWM_FREQUENCY_MAX); // constrain to 40kHz max - centered pwm
                                        // has twice lower frequency
  bldc_6pwm_motor_slots_t m_slot = {};
  // determine which motor are we connecting
  // and set the appropriate configuration parameters
  int slot_num;
  for (slot_num = 0; slot_num < 2; slot_num++) {
    if (esp32_bldc_6pwm_motor_slots[slot_num].pinAH ==
        _EMPTY_SLOT) { // put the new motor in the first empty slot
...
void _writeDutyCycle6PWM(float dc_a, float dc_b, float dc_c, float dead_zone,
  ...
  for (int i = 0; i < 2; i++) {
    if (esp32_bldc_6pwm_motor_slots[i].pinAH == pinA_h) { // if motor slot found
      ...
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
                     MCPWM_OPR_A, dc_a * 100.0);
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_0,
                     MCPWM_OPR_B, dc_a * 100.0);
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
                     MCPWM_OPR_A, dc_b * 100.0);
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_1,
                     MCPWM_OPR_B, dc_b * 100.0);
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_2,
                     MCPWM_OPR_A, dc_c * 100.0);
      mcpwm_set_duty(esp32_bldc_6pwm_motor_slots[i].mcpwm_unit, MCPWM_TIMER_2,
                     MCPWM_OPR_B, dc_c * 100.0);
      break;
    }
  }
}
