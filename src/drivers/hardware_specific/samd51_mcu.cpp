...
//	TCC#   Channels   WO_NUM   Counter size   Fault   Dithering   Output
// matrix   DTI   SWAP   Pattern generation 	0         6         8 24-bit Yes
// Yes       Yes            Yes   Yes    Yes 	1         4         8 24-bit Yes
// Yes       Yes            Yes   Yes    Yes 	2         3 3         16-bit Yes
// -         Yes            -     -      - 	3 2 2         16-bit       Yes
// -         -              -     -      - 	4 2 2         16-bit       Yes
// -         -              -     -      -
...
wo_association ASSOCIATION_NOT_FOUND = {NOT_A_PORT,   0, NOT_ON_TIMER, 0,
                                        NOT_ON_TIMER, 0, NOT_ON_TIMER, 0};

uint8_t TCC_CHANNEL_COUNT[] = {TCC0_CC_NUM, TCC1_CC_NUM, TCC2_CC_NUM,
                               TCC3_CC_NUM, TCC4_CC_NUM};
...
EPioType getPeripheralOfPermutation(int permutation, int pin_position) {
  return ((permutation >> pin_position) & 0x01) == 0x1 ? PIO_TIMER_ALT
                                                       : PIO_TIMER;
}
...
void writeSAMDDutyCycle(int chaninfo, float dc) {
  ...
  if (tccn < TCC_INST_NUM) {
    ...
    while ((tcc->SYNCBUSY.vec.CC & (0x1 << chan)) > 0)
      ;
    tcc->CCBUF[chan].reg = (uint32_t)((SIMPLEFOC_SAMD_PWM_RESOLUTION - 1) *
                                      dc); // TODO pwm frequency!
    //		tcc->STATUS.vec.CCBUFV |= (0x1<<chan);
    //		while ( tcc->SYNCBUSY.bit.STATUS > 0 );
    //		tcc->CTRLBSET.reg |=
    // TCC_CTRLBSET_CMD(TCC_CTRLBSET_CMD_UPDATE_Val); 		while (
    // tcc->SYNCBUSY.bit.CTRLB > 0 );
  } else {
    // Tc* tc = (Tc*)GetTC(chaninfo);
    // //tc->COUNT16.CC[chan].reg = (uint32_t)((SIMPLEFOC_SAMD_PWM_RESOLUTION-1)
    // * dc); tc->COUNT8.CC[chan].reg =
    // (uint8_t)((SIMPLEFOC_SAMD_PWM_TC_RESOLUTION-1) * dc); while (
    // tc->COUNT8.STATUS.bit.SYNCBUSY == 1 );
  }
}
...
/**
 * Configure Clock 4 - we want all simplefoc PWMs to use the same clock. This
 * ensures that any compatible pin combination can be used without having to
 * worry about configuring different clocks.
 */
void configureSAMDClock() {
  if (!SAMDClockConfigured) {
    SAMDClockConfigured = true; // mark clock as configured
    for (int i = 0; i < TCC_INST_NUM;
         i++) // mark all the TCCs as not yet configured
      tccConfigured[i] = false;

    // GCLK->GENCTRL[DPLL_CLOCK_NUM].reg = GCLK_GENCTRL_GENEN |
    // GCLK_GENCTRL_DIV(1)
    // | GCLK_GENCTRL_SRC(GCLK_GENCTRL_SRC_DFLL_Val); while
    // (GCLK->SYNCBUSY.vec.GENCTRL&(0x1<<DPLL_CLOCK_NUM));

    // GCLK->PCHCTRL[1].reg =
    // GCLK_PCHCTRL_GEN(DPLL_CLOCK_NUM)|GCLK_PCHCTRL_CHEN; while
    // (GCLK->SYNCBUSY.vec.GENCTRL&(0x1<<DPLL_CLOCK_NUM));

    // OSCCTRL->Dpll[0].DPLLCTRLA.bit.ENABLE = 0;
    // while (OSCCTRL->Dpll[0].DPLLSYNCBUSY.reg!=0x0);

    // OSCCTRL->Dpll[0].DPLLCTRLB.bit.REFCLK =
    // OSCCTRL_DPLLCTRLB_REFCLK_GCLK_Val; while
    // (OSCCTRL->Dpll[0].DPLLSYNCBUSY.reg!=0x0); OSCCTRL->Dpll[0].DPLLRATIO.reg
    // = 3; while (OSCCTRL->Dpll[0].DPLLSYNCBUSY.reg!=0x0);
    ...
    GCLK->GENCTRL[PWM_CLOCK_NUM].reg =
        GCLK_GENCTRL_GENEN | GCLK_GENCTRL_DIV(1) | GCLK_GENCTRL_IDC |
 ...
 */
void configureTCC(tccConfiguration &tccConfig, long pwm_frequency, bool negate,
                  float hw6pwm) {
  ...
  if (!tccConfigured[tccConfig.tcc.tccn]) {
    ...
    GCLK->PCHCTRL[GCLK_CLKCTRL_ID_ofthistcc].reg =
        GCLK_PCHCTRL_GEN(PWM_CLOCK_NUM) | GCLK_PCHCTRL_CHEN;
    while (GCLK->SYNCBUSY.vec.GENCTRL & (0x1 << PWM_CLOCK_NUM))
      ;
  }
  ...
  if (tccConfig.tcc.tccn < TCC_INST_NUM) {
    ...
    uint8_t invenMask =
        ~(1 << tccConfig.tcc.chan); // negate (invert) the signal if needed
    ...
    if (!tccConfigured[tccConfig.tcc.tccn]) {
      tcc->WAVE.reg |=
          TCC_WAVE_POL(0xF) |
          TCC_WAVE_WAVEGEN_DSTOP; // Set wave form configuration - TODO check
                                  // this... why set like this?
      while (tcc->SYNCBUSY.bit.WAVE == 1)
        ; // wait for sync

      tcc->PER.reg = SIMPLEFOC_SAMD_PWM_RESOLUTION -
                     1; // Set counter Top using the PER register
      while (tcc->SYNCBUSY.bit.PER == 1)
        ; // wait for sync
    ...
    tcc->CTRLA.reg |=
        TCC_CTRLA_ENABLE |
        TCC_CTRLA_PRESCALER_DIV1; // 48Mhz/1=48Mhz/2(up/down)=24MHz/1024=24KHz
    while (tcc->SYNCBUSY.bit.ENABLE == 1)
      ; // wait for sync
    ...
    // // unfortunately we need the 8-bit counter mode to use the PER
    // register... tc->COUNT8.CTRLA.reg |= TC_CTRLA_MODE_COUNT8 |
    // TC_CTRLA_WAVEGEN_NPWM ; while ( tc->COUNT8.STATUS.bit.SYNCBUSY == 1 );
    // // meaning prescaler of 8, since TC-Unit has no up/down mode, and has
    // resolution of 250 rather than 1000... tc->COUNT8.CTRLA.bit.PRESCALER =
    // TC_CTRLA_PRESCALER_DIV8_Val ; while ( tc->COUNT8.STATUS.bit.SYNCBUSY == 1
    // );
