...
tccConfiguration getTCCChannelNr(int pin, EPioType peripheral) {
  ...
  struct wo_association &association =
      getWOAssociation(pinDesc.ulPort, pinDesc.ulPin);
...
bool checkPeripheralPermutationSameTCC6(
    tccConfiguration &pinAh, tccConfiguration &pinAl, tccConfiguration &pinBh,
    tccConfiguration &pinBl, tccConfiguration &pinCh, tccConfiguration &pinCl) {
  if (inUse(pinAh) || inUse(pinAl) || inUse(pinBh) || inUse(pinBl) ||
      inUse(pinCh) || inUse(pinCl))
    return false;

  if (pinAh.tcc.tccn < 0 || pinAh.tcc.tccn != pinAl.tcc.tccn ||
      pinAh.tcc.tccn != pinBh.tcc.tccn || pinAh.tcc.tccn != pinBl.tcc.tccn ||
      pinAh.tcc.tccn != pinCh.tcc.tccn || pinAh.tcc.tccn != pinCl.tcc.tccn ||
      pinAh.tcc.tccn >= TCC_INST_NUM)
    return false;

  if (pinAh.tcc.chan == pinBh.tcc.chan || pinAh.tcc.chan == pinBl.tcc.chan ||
      pinAh.tcc.chan == pinCh.tcc.chan || pinAh.tcc.chan == pinCl.tcc.chan)
    return false;
  if (pinBh.tcc.chan == pinCh.tcc.chan || pinBh.tcc.chan == pinCl.tcc.chan)
    return false;
  if (pinAl.tcc.chan == pinBh.tcc.chan || pinAl.tcc.chan == pinBl.tcc.chan ||
      pinAl.tcc.chan == pinCh.tcc.chan || pinAl.tcc.chan == pinCl.tcc.chan)
    return false;
  if (pinBl.tcc.chan == pinCh.tcc.chan || pinBl.tcc.chan == pinCl.tcc.chan)
    return false;

  if (pinAh.tcc.chan != pinAl.tcc.chan || pinBh.tcc.chan != pinBl.tcc.chan ||
      pinCh.tcc.chan != pinCl.tcc.chan)
    return false;
...
bool checkPeripheralPermutationCompatible(tccConfiguration pins[],
                                          uint8_t num) {
  for (int i = 0; i < num; i++)
...
bool checkPeripheralPermutationCompatible6(
    tccConfiguration &pinAh, tccConfiguration &pinAl, tccConfiguration &pinBh,
    tccConfiguration &pinBl, tccConfiguration &pinCh, tccConfiguration &pinCl) {
  // check we're valid PWM pins
  if (pinAh.tcc.tccn < 0 || pinAl.tcc.tccn < 0 || pinBh.tcc.tccn < 0 ||
      pinBl.tcc.tccn < 0 || pinCh.tcc.tccn < 0 || pinCl.tcc.tccn < 0)
    return false;
  // only TCC units for 6-PWM
  if (pinAh.tcc.tccn >= TCC_INST_NUM || pinAl.tcc.tccn >= TCC_INST_NUM ||
      pinBh.tcc.tccn >= TCC_INST_NUM || pinBl.tcc.tccn >= TCC_INST_NUM ||
      pinCh.tcc.tccn >= TCC_INST_NUM || pinCl.tcc.tccn >= TCC_INST_NUM)
    return false;
  ...
  if (pinAh.tcc.chaninfo == pinBh.tcc.chaninfo ||
      pinAh.tcc.chaninfo == pinBl.tcc.chaninfo ||
      pinAh.tcc.chaninfo == pinCh.tcc.chaninfo ||
      pinAh.tcc.chaninfo == pinCl.tcc.chaninfo)
    return false;
  if (pinAl.tcc.chaninfo == pinBh.tcc.chaninfo ||
      pinAl.tcc.chaninfo == pinBl.tcc.chaninfo ||
      pinAl.tcc.chaninfo == pinCh.tcc.chaninfo ||
      pinAl.tcc.chaninfo == pinCl.tcc.chaninfo)
    return false;
  if (pinBh.tcc.chaninfo == pinCh.tcc.chaninfo ||
      pinBh.tcc.chaninfo == pinCl.tcc.chaninfo)
    return false;
  if (pinBl.tcc.chaninfo == pinCh.tcc.chaninfo ||
      pinBl.tcc.chaninfo == pinCl.tcc.chaninfo)
    return false;

  // check H/L pins are on same timer
  if (pinAh.tcc.tccn != pinAl.tcc.tccn || pinBh.tcc.tccn != pinBl.tcc.tccn ||
      pinCh.tcc.tccn != pinCl.tcc.tccn)
    return false;
...
int checkHardware6PWM(const int pinA_h, const int pinA_l, const int pinB_h,
                      const int pinB_l, const int pinC_h, const int pinC_l) {
  for (int i = 0; i < 64; i++) {
    tccConfiguration pinAh =
        getTCCChannelNr(pinA_h, getPeripheralOfPermutation(i, 0));
    tccConfiguration pinAl =
        getTCCChannelNr(pinA_l, getPeripheralOfPermutation(i, 1));
    tccConfiguration pinBh =
        getTCCChannelNr(pinB_h, getPeripheralOfPermutation(i, 2));
    tccConfiguration pinBl =
        getTCCChannelNr(pinB_l, getPeripheralOfPermutation(i, 3));
    tccConfiguration pinCh =
        getTCCChannelNr(pinC_h, getPeripheralOfPermutation(i, 4));
    tccConfiguration pinCl =
        getTCCChannelNr(pinC_l, getPeripheralOfPermutation(i, 5));
    if (checkPeripheralPermutationSameTCC6(pinAh, pinAl, pinBh, pinBl, pinCh,
                                           pinCl))
      return i;
  }
  return -1;
...
int checkSoftware6PWM(const int pinA_h, const int pinA_l, const int pinB_h,
                      const int pinB_l, const int pinC_h, const int pinC_l) {
  for (int i = 0; i < 64; i++) {
    tccConfiguration pinAh =
        getTCCChannelNr(pinA_h, getPeripheralOfPermutation(i, 0));
    ...
    tccConfiguration pinCl =
        getTCCChannelNr(pinC_l, getPeripheralOfPermutation(i, 5));
    if (checkPeripheralPermutationCompatible6(pinAh, pinAl, pinBh, pinBl, pinCh,
                                              pinCl))
      return i;
  }
  return -1;
...
int checkPermutations(uint8_t num, int pins[],
...
void _configure2PWM(long pwm_frequency, const int pinA, const int pinB) {
  ...
  int compatibility =
      checkPermutations(2, pins, checkPeripheralPermutationCompatible);
  if (compatibility < 0) {
    // no result!
#ifdef SIMPLEFOC_SAMD_DEBUG
  ...
  attachTCC(tccConfs[0]); // in theory this can fail, but there is no way to
                          // signal it...
  ...
  // set up clock - Note: if we did this right it should be possible to get all
  // TCC units synchronized? e.g. attach all the timers, start them, and then
  // start the clock... but this would require API-changes in SimpleFOC...
  configureSAMDClock();
 ...
 * i.e. 8 different pins can be used, but only 4 different signals (WO[x]) on
 * those 8 pins WO[0] and WO[4] are the same WO[1] and WO[5] are the same WO[2]
 * and WO[6] are the same WO[3] and WO[7] are the same
 *
 * If you're on the Arduino Nano33 IoT, please see the Nano33 IoT pinout diagram
 * to see which TCC0/WO[x] signal is on which pin of the Nano. You can drive one
 * motor on TCC0. For other boards, consult their documentation.
 *
 * Note:
 * That's if we want to keep the signals strictly in sync.
 *
 * If we can accept out-of-sync PWMs on the different phases, we could drive up
 * to 4 BLDCs in 3-PWM mode, using all the TCC channels. (TCC0 & TCC3 - 4
 * channels each, TCC1 & TCC2 - 2 channels each)
 *
 * All channels will use the same resolution, prescaler and clock, but they will
 * have different start-times leading to misaligned signals.
 *
 *
 * @param pwm_frequency - frequency in hertz - if applicable
 * @param pinA pinA bldc driver
...
void _configure3PWM(long pwm_frequency, const int pinA, const int pinB,
  ...
  int compatibility =
      checkPermutations(3, pins, checkPeripheralPermutationCompatible);
  if (compatibility < 0) {
    // no result!
#ifdef SIMPLEFOC_SAMD_DEBUG
    SIMPLEFOC_SAMD_DEBUG_SERIAL.println("Bad combination!");
 ...
 */
void _configure4PWM(long pwm_frequency, const int pin1A, const int pin1B,
                    const int pin2A, const int pin2B) {
#ifdef SIMPLEFOC_SAMD_DEBUG
  printAllPinInfos();
#endif
  int pins[4] = {pin1A, pin1B, pin2A, pin2B};
  int compatibility =
      checkPermutations(4, pins, checkPeripheralPermutationCompatible);
  if (compatibility < 0) {
    // no result!
#ifdef SIMPLEFOC_SAMD_DEBUG
    SIMPLEFOC_SAMD_DEBUG_SERIAL.println("Bad combination!");
 ...
 *
 * If you're on the Arduino Nano33 IoT, please see the Nano33 IoT pinout diagram
 * to see which TCC0/WO[x] signal is on which pin of the Nano. You can drive 1
 * BLDC on TCC0. For other boards, consult their documentation.
 *
 *
 * @param pwm_frequency - frequency in hertz - if applicable
 * @param dead_zone  duty cycle protection zone [0, 1] - both low and high side
 * low - if applicable
 ...
 */
int _configure6PWM(long pwm_frequency, float dead_zone, const int pinA_h,
                   const int pinA_l, const int pinB_h, const int pinB_l,
                   const int pinC_h, const int pinC_l) {
  // we want to use a TCC channel with 1 non-inverted and 1 inverted output for
  // each phase, with dead-time insertion
#ifdef SIMPLEFOC_SAMD_DEBUG
  printAllPinInfos();
#endif
  int compatibility =
      checkHardware6PWM(pinA_h, pinA_l, pinB_h, pinB_l, pinC_h, pinC_l);
  if (compatibility < 0) {
    compatibility =
        checkSoftware6PWM(pinA_h, pinA_l, pinB_h, pinB_l, pinC_h, pinC_l);
    if (compatibility < 0) {
      // no result!
#ifdef SIMPLEFOC_SAMD_DEBUG
      SIMPLEFOC_SAMD_DEBUG_SERIAL.println("Bad combination!");
  ...
  tccConfiguration pinAh =
      getTCCChannelNr(pinA_h, getPeripheralOfPermutation(compatibility, 0));
  tccConfiguration pinAl =
      getTCCChannelNr(pinA_l, getPeripheralOfPermutation(compatibility, 1));
  tccConfiguration pinBh =
      getTCCChannelNr(pinB_h, getPeripheralOfPermutation(compatibility, 2));
  tccConfiguration pinBl =
      getTCCChannelNr(pinB_l, getPeripheralOfPermutation(compatibility, 3));
  tccConfiguration pinCh =
      getTCCChannelNr(pinC_h, getPeripheralOfPermutation(compatibility, 4));
  tccConfiguration pinCl =
      getTCCChannelNr(pinC_l, getPeripheralOfPermutation(compatibility, 5));
  ...
  allAttached |= attachTCC(
      pinAh); // in theory this can fail, but there is no way to signal it...
  ...
  SIMPLEFOC_SAMD_DEBUG_SERIAL.println("Attached pins...");
#endif
  // set up clock - if we did this right it should be possible to get all TCC
  // units synchronized? e.g. attach all the timers, start them, and then start
  // the clock... but this would require API changes in SimpleFOC driver API
  configureSAMDClock();

  // configure the TCC(s)
  configureTCC(pinAh, pwm_frequency, false,
               (pinAh.tcc.chaninfo == pinAl.tcc.chaninfo) ? dead_zone : -1);
  if ((pinAh.tcc.chaninfo != pinAl.tcc.chaninfo))
    configureTCC(pinAl, pwm_frequency, true, -1.0);
  configureTCC(pinBh, pwm_frequency, false,
               (pinBh.tcc.chaninfo == pinBl.tcc.chaninfo) ? dead_zone : -1);
  if ((pinBh.tcc.chaninfo != pinBl.tcc.chaninfo))
    configureTCC(pinBl, pwm_frequency, true, -1.0);
  configureTCC(pinCh, pwm_frequency, false,
               (pinCh.tcc.chaninfo == pinCl.tcc.chaninfo) ? dead_zone : -1);
 ...
 */
void _writeDutyCycle4PWM(float dc_1a, float dc_1b, float dc_2a, float dc_2b,
                         int pin1A, int pin1B, int pin2A, int pin2B) {
 ...
 *       the low side pins are automatically driven by the SAMD DTI module, so
 * it is enough to set the high-side duty cycle. No sanity checks are perfomed
 * to ensure the pinA, pinB, pinC are the same pins you used in configure
 * method... so use appropriately.
 *
 * @param dc_a  duty cycle phase A [0, 1]
 * @param dc_b  duty cycle phase B [0, 1]
 * @param dc_c  duty cycle phase C [0, 1]
 ...
 *
 */
void _writeDutyCycle6PWM(float dc_a, float dc_b, float dc_c, float dead_zone,
                         int pinA_h, int pinA_l, int pinB_h, int pinB_l,
  ...
  if (tcc1->tcc.chaninfo != tcc2->tcc.chaninfo) {
    ...
    if (ls > 1.0)
      ls = 1.0; // no off-time is better than too-short dead-time
    writeSAMDDutyCycle(tcc1->tcc.chaninfo, dc_a);
    writeSAMDDutyCycle(tcc2->tcc.chaninfo, ls);
  } else
    writeSAMDDutyCycle(tcc1->tcc.chaninfo,
                       dc_a); // dead-time is done is hardware, no need to set
                              // low side pin explicitly
...
void printAllPinInfos() {
  ...
  for (uint8_t pin = 0; pin < PINS_COUNT; pin++) {
    const PinDescription &pinDesc = g_APinDescription[pin];
    wo_association &association =
        getWOAssociation(pinDesc.ulPort, pinDesc.ulPin);
    SIMPLEFOC_SAMD_DEBUG_SERIAL.print("Pin ");
    if (pin < 10)
      SIMPLEFOC_SAMD_DEBUG_SERIAL.print("0");
    SIMPLEFOC_SAMD_DEBUG_SERIAL.print(pin);
