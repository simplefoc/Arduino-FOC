...
// until I can figure out if this can be quickly read from some register, keep
// it here. it also serves as a marker for what slices are already used.
...
void setupPWM(int pin, long pwm_frequency, bool invert = false) {
  ...
  if (wrapvalue < 999)
    wrapvalue = 999; // 66kHz, resolution 1000
  if (wrapvalue > 3299)
    wrapvalue = 3299; // 20kHz, resolution 3300
#ifdef SIMPLEFOC_DEBUG_RP2040
  ...
  if (invert) {
    if (chan == 0)
      hw_write_masked(&pwm_hw->slice[slice].csr, 0x1 << PWM_CH0_CSR_A_INV_LSB,
                      PWM_CH0_CSR_A_INV_BITS);
    else
      hw_write_masked(&pwm_hw->slice[slice].csr, 0x1 << PWM_CH0_CSR_B_INV_LSB,
                      PWM_CH0_CSR_B_INV_BITS);
  }
