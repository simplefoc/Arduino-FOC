 ...
 * Good for SAMD2xE, SAMD2xG and SAMD2xJ devices. Other SAMD21s currently not
 * supported in arduino anyway?
 *
 * Note: only the pins which have timers associated are listed in this table.
 * You can use the values from g_APinDescription.ulPort and
 * g_APinDescription.ulPin to find the correct row in the table.
 *
...
void configureSAMDClock() {
  ...
  //      would enable 48KHz PWM clocks, and setting the frequency between 24Khz
  //      with resolution 2000, to 48KHz with resolution 1000
  ...
  if (!SAMDClockConfigured) {
    ...
    REG_GCLK_GENDIV =
        GCLK_GENDIV_DIV(
            1) | // Divide the 48MHz clock source by divisor N=1: 48MHz/1=48MHz
...
void configureTCC(tccConfiguration &tccConfig, long pwm_frequency, bool negate,
  ...
  if (!tccConfigured[tccConfig.tcc.tccn]) {
    ...
    if (tccConfig.tcc.tccn >= TCC_INST_NUM) {
      ...
      // resolution of 250 rather than 1000...
      ...
      tcc->WAVE.reg |= TCC_WAVE_POL(0xF) |
                       TCC_WAVEB_WAVEGENB_DSBOTH; // Set wave form configuration
      ...
      uint8_t chanCount =
          (tccConfig.tcc.tccn == 1 || tccConfig.tcc.tccn == 2) ? 2 : 4;
...
void writeSAMDDutyCycle(int chaninfo, float dc) {
  ...
  if (tccn < TCC_INST_NUM) {
    ...
    // set via CC
    //		tcc->CC[chan].reg = (uint32_t)((SIMPLEFOC_SAMD_PWM_RESOLUTION-1)
    //* dc); 		uint32_t chanbit = 0x1<<(TCC_SYNCBUSY_CC0_Pos+chan);
    // while ( (tcc->SYNCBUSY.reg & chanbit) > 0 );
    // set via CCB
    ...
    tc->COUNT8.CC[chan].reg =
        (uint8_t)((SIMPLEFOC_SAMD_PWM_TC_RESOLUTION - 1) * dc);
    while (tc->COUNT8.STATUS.bit.SYNCBUSY == 1)
      ;
  }
}
