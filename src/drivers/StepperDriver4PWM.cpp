...
StepperDriver4PWM::StepperDriver4PWM(int ph1A, int ph1B, int ph2A, int ph2B,
                                     int en1, int en2) {
  // Pin initialization
  pwm1A = ph1A;
  pwm1B = ph1B;
...
void StepperDriver4PWM::setPwm(float Ualpha, float Ubeta) {
  float duty_cycle1A(0.0), duty_cycle1B(0.0), duty_cycle2A(0.0),
      duty_cycle2B(0.0);
  ...
  // write to hardware
  _writeDutyCycle4PWM(duty_cycle1A, duty_cycle1B, duty_cycle2A, duty_cycle2B,
