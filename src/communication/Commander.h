...
public:
  ...
  // printing variables
  VerboseMode verbose =
      VerboseMode::user_friendly; //!< flag signaling that the commands should
                                  //!< output user understanable text
  uint8_t decimal_places =
      3; //!< number of decimal places to be used when displaying numbers
   ...
   * file) 'Q' - Q current PID controller & LPF (see function pid and lpf for
   * commands) 'D' - D current PID controller & LPF (see function pid and lpf
   * for commands) 'V' - Velocity PID controller & LPF  (see function pid and
   * lpf for commands) 'A' - Angle PID controller & LPF     (see function pid
   * and lpf for commands) 'L' - Limits sub-commands: 'C' - Current 'U' -
   * Voltage 'V' - Velocity 'C' - Motion control type config sub-commands: 'D' -
   * downsample motiron loop '0' - torque '1' - velocity '2' - angle 'T' -
   * Torque control type sub-commands: '0' - voltage '1' - current '2' -
   * foc_current 'E' - Motor status (enable/disable) sub-commands: '0' - enable
   *          '1' - disable
   *    'R' - Motor resistance
   *    'S' - Sensor offsets
   *          sub-commands:
   ...
   *    '' - Target get/set
   *
   *  - Each of them can be get by sening the command letter -(ex. 'R' - to get
   * the phase resistance)
   *  - Each of them can be set by sending 'IdSubidValue' - (ex. SM1.5 for
   * setting sensor zero offset to 1.5)
   *
   */
...
private:
  ...
  CommandCallback call_list[20]; //!< array of command callback pointers - 20 is
                                 //!< an arbitrary number
  ...
  char received_chars[MAX_COMMAND_LENGTH] = {
