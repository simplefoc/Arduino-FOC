...
void HallSensor::updateState() {
  ...
  // glitch avoidance #1 - sometimes we get an interrupt but pins haven't
  // changed
  ...
  if (new_electric_sector - electric_sector > 3) {
    ...
    direction = (new_electric_sector > electric_sector) ? Direction::CW
                                                        : Direction::CCW;
  }
  electric_sector = new_electric_sector;

  // glitch avoidance #2 changes in direction can cause velocity spikes.
  // Possible improvements needed in this area
