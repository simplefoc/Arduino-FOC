 ...
 * @param _min_raw_count  the smallest expected reading.  Whilst you might
 * expect it to be 0 it is often ~15.  Getting this wrong results in a small
 * click once per revolution
 * @param _max_raw_count  the largest value read.  whilst you might expect it to
 * be 2^10 = 1023 it is often ~ 1020. Note: For ESP32 (with 12bit ADC the value
 * will be nearer 4096)
 */
...
float MagneticSensorAnalog::getAngle() {
  ...
  if (abs(delta) > (0.8 * cpr))
    full_rotation_offset += delta > 0 ? -_2PI : _2PI;
...
float MagneticSensorAnalog::getVelocity() {
  // TODO: Refactor?: to avoid angle being called twice, velocity is
  // pre-calculted during getAngle
  return velocity;
}
