 ...
 */
MagneticSensorPWM::MagneticSensorPWM(uint8_t _pinPWM, int _min_raw_count,
                                     int _max_raw_count) {
...
//  get velocity (rad/s)
float MagneticSensorPWM::getVelocity() { return velocity; }

// read the raw counter of the magnetic sensor
int MagneticSensorPWM::getRawCount() {
  if (!is_interrupt_based) { // if it's not interrupt based read the value in a
                             // blocking way
...
void MagneticSensorPWM::enableInterrupt(void (*doPWM)()) {
  ...
  is_interrupt_based = true;
    ...
    !defined(__AVR_ATmega2560__) // if mcu is not atmega328 && if mcu is not
                                 // atmega2560
