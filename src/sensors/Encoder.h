...
public:
  ...
  */
  ...
  void init();
  /**
   *  function enabling hardware interrupts for the encoder channels with
...
private:
  int hasIndex(); //!< function returning 1 if encoder has index pin and 0 if
                  //!< not.
  ...
  volatile bool index_found =
      false; //!< flag stating that the index has been found
