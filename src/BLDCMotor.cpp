...
void BLDCMotor::init() {
  ...
  // if no current sensing and the user has set the phase resistance of the
  // motor use current limit to calculate the voltage limit
  if (!current_sense && _isset(phase_resistance)) {
    float new_voltage_limit =
        current_limit * (phase_resistance); // v_lim = current_lim / (3/2 phase
                                            // resistance) - worst case
    // use it if it is less then voltage_limit set by the user
    voltage_limit =
        new_voltage_limit < voltage_limit ? new_voltage_limit : voltage_limit;
  }
...
int BLDCMotor::initFOC(float zero_electric_offset,
                       Direction _sensor_direction) {
  int exit_flag = 1;
...
int BLDCMotor::alignSensor() {
  ...
  if (!_isset(sensor_direction)) {
    ...
    if (fabs(moved * pole_pairs - _2PI) >
        0.5) { // 0.5 is arbitrary number it can be lower or higher!
      if (monitor_port)
  ...
  if (!_isset(zero_electric_angle)) {
    ...
    zero_electric_angle = _normalizeAngle(
        _electricalAngle(sensor_direction * sensor->getAngle(), pole_pairs));
    _delay(20);
    if (monitor_port) {
...
void BLDCMotor::loopFOC() {
  ...
  if (controller == MotionControlType::angle_openloop ||
    ...
    return;
  // shaft angle
  shaft_angle = shaftAngle(); // read value even if motor is disabled to keep
                              // the monitoring updated
...
void BLDCMotor::move(float new_target) {
  ...
  shaft_velocity = shaftVelocity(); // read value even if motor is disabled to
                                    // keep the monitoring updated
  ...
  switch (controller) {
    case MotionControlType::torque:
      if (torque_controller ==
          TorqueControlType::voltage) // if voltage torque control
    ...
    case MotionControlType::angle:
      ...
      current_sp = PID_velocity(shaft_velocity_sp -
                                shaft_velocity); // if voltage torque control
    ...
    case MotionControlType::velocity:
      ...
      // calculate the torque command
      current_sp =
          PID_velocity(shaft_velocity_sp -
                       shaft_velocity); // if current/foc_current torque control
    ...
    case MotionControlType::velocity_openloop:
      ...
      shaft_velocity_sp = target;
      voltage.q = velocityOpenloop(
          shaft_velocity_sp); // returns the voltage that is set to the motor
      voltage.d = 0;
      break;
    case MotionControlType::angle_openloop:
      // angle control in open loop
      shaft_angle_sp = target;
      voltage.q = angleOpenloop(
          shaft_angle_sp); // returns the voltage that is set to the motor
      voltage.d = 0;
      break;
  }
}
  ...
  switch (foc_modulation) {
    case FOCModulationType::Trapezoid_120:
      ...
      static int trap_120_map[6][3] = {
          ...
          {_HIGH_IMPEDANCE, -1, 1},
          {1, -1, _HIGH_IMPEDANCE},
          {1, _HIGH_IMPEDANCE, -1} // each is 60 degrees with values for 3
                                   // phases of 1=positive -1=negative 0=high-z
      };
      // static int trap_120_state = 0;
      sector = 6 * (_normalizeAngle(angle_el + _PI_6) /
      ...
      // modulation_centered == false > or Adaptable centering, all phases drawn
      // to 0 when Uq=0
      ...
      if (trap_120_map[sector][0] == _HIGH_IMPEDANCE) {
        ...
        driver->setPhaseState(_HIGH_IMPEDANCE, _ACTIVE,
                              _ACTIVE); // disable phase if possible
      } else if (trap_120_map[sector][1] == _HIGH_IMPEDANCE) {
        Ua = trap_120_map[sector][0] * Uq + center;
        Ub = center;
        Uc = trap_120_map[sector][2] * Uq + center;
        driver->setPhaseState(_ACTIVE, _HIGH_IMPEDANCE,
                              _ACTIVE); // disable phase if possible
      } else {
        Ua = trap_120_map[sector][0] * Uq + center;
        Ub = trap_120_map[sector][1] * Uq + center;
        Uc = center;
        driver->setPhaseState(_ACTIVE, _ACTIVE,
                              _HIGH_IMPEDANCE); // disable phase if possible
      }
    ...
    case FOCModulationType::Trapezoid_150:
      ...
      static int trap_150_map[12][3] = {
          ...
          {1, -1, -1},
          {1, _HIGH_IMPEDANCE, -1},
          {1, 1, -1} // each is 30 degrees with values for 3 phases of
                     // 1=positive -1=negative 0=high-z
      };
      // static int trap_150_state = 0;
      sector = 12 * (_normalizeAngle(angle_el + _PI_6) /
                     _2PI); // adding PI/6 to align with other modes
      // centering the voltages around either
      // modulation_centered == true > driver.volage_limit/2
      // modulation_centered == false > or Adaptable centering, all phases drawn
