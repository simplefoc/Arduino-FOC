#include "../hardware_api.h"
    ...
    defined(__AVR_ATmega2560__) //  if mcu is atmega328 or atmega2560
...
void _configureADCInline(const int pinA, const int pinB, const int pinC) {
  ...
  if (_isset(pinC))
    ...
    defined(__AVR_ATmega328PB__) ||                                            \
    defined(__AVR_ATmega2560__) //  if mcu is atmega328 or atmega2560
  // set hight frequency adc - ADPS2,ADPS1,ADPS0 | 001 (16mhz/2) | 010 ( 16mhz/4
  // ) | 011 (16mhz/8) | 100(16mhz/16) | 101 (16mhz/32) | 110 (16mhz/64) | 111
  // (16mhz/128 default) set divisor to 8 - adc frequency 16mhz/8 = 2 mhz
  // arduino takes 25 conversions per sample so - 2mhz/25 = 80k samples per
  // second - 12.5us per sample
  cbi(ADCSRA, ADPS2);
  sbi(ADCSRA, ADPS1);
  sbi(ADCSRA, ADPS0);
#endif
