...
static uint16_t
    a = 0xFFFF,
    b = 0xFFFF,
    c = 0xFFFF; // updated by adcStopWithDMA when configured in freerunning mode
...
void _driverSyncLowSide() {
  SIMPLEFOC_SAMD_DEBUG_SERIAL.println(
      F("TODO! _driverSyncLowSide() is not implemented"));
  instance.startADCScan();
  // TODO: hook with PWM interrupts
}

// Credit: significant portions of this code were pulled from Paul Gould's git
...
void SAMDCurrentSenseADCDMA::init(int pinA, int pinB, int pinC, int pinAREF,
                                  float voltageAREF, uint32_t adcBits,
                                  uint32_t channelDMA) {
  this->pinA = pinA;
  this->pinB = pinB;
  this->pinC = pinC;
  ...
  initADC();
  initDMA();
  startADCScan(); // so we have something to read next time we call
                  // readResults()
}
...
bool SAMDCurrentSenseADCDMA::readResults(uint16_t &a, uint16_t &b,
                                         uint16_t &c) {
...
void SAMDCurrentSenseADCDMA::initADC() {
  ...
  ADCsync();
  // ADC->REFCTRL.bit.REFSEL = ADC_REFCTRL_REFSEL_INTVCC0_Val; //  2.2297 V
  // Supply VDDANA
  ...
  This register gives the number of input sources included in the pin scan. The
  number of input sources included is INPUTSCAN + 1. The input channels included
  are in the range from MUXPOS + INPUTOFFSET to MUXPOS + INPUTOFFSET +
  INPUTSCAN. The range of the scan mode must not exceed the number of input
  channels available on the device. Bits 4:0 – MUXPOS[4:0]: Positive Mux Input
  Selection These bits define the Mux selection for the positive ADC input.
  Table 32-14 shows the possible input selections. If the internal bandgap
  voltage or temperature sensor input channel is selected, then the Sampling
  Time Length bit group in the SamplingControl register must be written. Table
  32-14. Positive Mux Input Selection MUXPOS[4:0] Group configuration
  Description 0x00 PIN0 ADC AIN0 pin 0x01 PIN1 ADC AIN1 pin 0x02 PIN2 ADC AIN2
  pin 0x03 PIN3 ADC AIN3 pin 0x04 PIN4 ADC AIN4 pin 0x05 PIN5 ADC AIN5 pin 0x06
  PIN6 ADC AIN6 pin 0x07 PIN7 ADC AIN7 pin 0x08 PIN8 ADC AIN8 pin 0x09 PIN9 ADC
  AIN9 pin 0x0A PIN10 ADC AIN10 pin 0x0B PIN11 ADC AIN11 pin 0x0C PIN12 ADC
  AIN12 pin 0x0D PIN13 ADC AIN13 pin 0x0E PIN14 ADC AIN14 pin 0x0F PIN15 ADC
  AIN15 pin 0x10 PIN16 ADC AIN16 pin 0x11 PIN17 ADC AIN17 pin 0x12 PIN18 ADC
  AIN18 pin 0x13 PIN19 ADC AIN19 pin 0x14-0x17 Reserved 0x18 TEMP Temperature
  reference 0x19 BANDGAP Bandgap voltage 0x1A SCALEDCOREVCC 1/4 scaled core
  supply 0x1B SCALEDIOVCC 1/4 scaled I/O supply 0x1C DAC DAC output 0x1D-0x1F
  Reserved
  */
  ADC->INPUTCTRL.bit.MUXPOS = oneBeforeFirstAIN;
  ADCsync();
  ADC->INPUTCTRL.bit.INPUTSCAN =
      lastAIN; // so the adc will scan from oneBeforeFirstAIN to lastAIN
               // (inclusive)
  ADCsync();
  ...
  ; // sample length in 1/2 CLK_ADC cycles, see GCLK_ADC and
    // ADC_CTRLB_PRESCALER_DIV16
  // according to the specsheet: f_GCLK_ADC ADC input clock frequency 48 MHz, so
  // same as fCPU
  ADCsync();
  ADC->CTRLB.reg =
      ADC_CTRLB_PRESCALER_DIV16 | ADC_CTRLB_FREERUN | ADC_CTRLB_RESSEL_12BIT;
  ADCsync();
}
