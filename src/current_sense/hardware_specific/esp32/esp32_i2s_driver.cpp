...
#if defined(ESP_H) && defined(ARDUINO_ARCH_ESP32) && defined(SOC_MCPWM_SUPPORTED) && !defined(SIMPLEFOC_ESP32_USELEDC) && ESP32_I2S_ADC

#pragma message()"SimpleFOC: Using ESP32 I2S ADC driver for current sensing")
...
{
    float adc_voltage_conv = ((ESP32CurrentSenseParams *)cs_params)->adc_voltage_conv;
...
void* IRAM_ATTR _configureI2S(const bool lowside, const void* driver_params, const int pinA, const int pinB, const int pinC){
  ...
  mcpwm_unit_t unit = ((ESP32CurrentSenseParams*)driver_params)->mcpwm_unit;
