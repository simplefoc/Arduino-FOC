...
LowsideCurrentSense::LowsideCurrentSense(float _shunt_resistor, float _gain,
...
void LowsideCurrentSense::calibrateOffsets() {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    if (_isset(pinC))
      offset_ic += _readADCVoltageLowSide(pinC);
    _delay(1);
  }
...
PhaseCurrent_s LowsideCurrentSense::getPhaseCurrents() {
  ...
  current.c = (!_isset(pinC))
                  ? 0
                  : (_readADCVoltageLowSide(pinC) - offset_ic) * gain_c; // amps
  return current;
}
